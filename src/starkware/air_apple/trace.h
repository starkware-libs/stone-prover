...
class Trace {
  ...
  */
  template <typename FieldElementT>
  static std::vector<std::vector<FieldElementT>> Allocate(uint64_t n_columns, uint64_t trace_length) {
    std::vector<std::vector<FieldElementT>> values;
    values.reserve(n_columns);
    for (uint64_t col = 0; col < n_columns; col++) {
  ...
  uint64_t Length() const { return values_[0].Size(); }

  uint64_t Width() const { return values_.size(); }
  const FieldElementVector& GetColumn(uint64_t column) const { return values_[column]; }

  std::vector<FieldElementVector> ConsumeAsColumnsVector() && { return std::move(values_); }

  void SetTraceElementForTesting(uint64_t column, uint64_t index, const FieldElement& field_element) {
