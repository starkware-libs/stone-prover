...
*/
inline uint64_t Dilute(uint64_t x, uint64_t spacing, uint64_t n_bits) {
  uint64_t res = 0;
...
*/
inline uint64_t Undilute(uint64_t x, uint64_t spacing, uint64_t n_bits) {
  uint64_t res = 0;
  for (uint64_t i = 0; i < n_bits; ++i) {
...
class DilutedCheckCell : public TableCheckCell<FieldElementT> {
 public:
  DilutedCheckCell(
      ...
      uint64_t spacing, uint64_t n_bits)
