...
Trace PermutationDummyAir<FieldElementT, LayoutId>::GetTrace() const {
  ...
  std::vector<uint64_t> perm_indices(this->trace_length_);
  for (uint64_t j = 0; j < this->trace_length_; j++) {
    perm_indices[j] = j;
  }
  // Permutate indices.
  ...
  for (uint64_t j = 0; j < this->trace_length_; j++) {
    for (uint64_t k = 0; k < this->kNOriginalCols; k++) {
...
Trace PermutationDummyAir<FieldElementT, LayoutId>::GetInteractionTrace(
  ...
  for (uint64_t i = 0; i < this->kNOriginalCols; ++i) {
    ASSERT_RELEASE(
