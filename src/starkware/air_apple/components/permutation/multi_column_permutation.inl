...
void MultiColumnPermutationComponent<FieldElementT>::WriteInteractionTrace(
  ...
  for (uint64_t series_i = 0; series_i < originals.size(); ++series_i) {
    ASSERT_RELEASE(
        originals[series_i].size() == perms[series_i].size(),
        "Number of original columns and perms columns is different.");

    const uint64_t input_cols_length = originals[series_i][0].size();

    for (uint64_t i = 0; i < originals.size(); ++i) {
      ASSERT_RELEASE(
    ...
    for (uint64_t i = 0; i < input_cols_length; ++i) {
      auto val_orig = originals[series_i][0][i];
      auto val_perm = perms[series_i][0][i];
      for (uint64_t elm_idx = 1; elm_idx < interaction_elms.size(); elm_idx++) {
