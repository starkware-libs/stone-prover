...
PermutationComponent<FieldElementT>::PermutationComponent(
    const std::string& name, const uint64_t n_series, const TraceGenerationContext& ctx)
    : n_series_(n_series) {
...
void PermutationComponent<FieldElementT>::WriteInteractionTrace(
  ...
  for (uint64_t series_i = 0; series_i < n_series_; ++series_i) {
    ASSERT_RELEASE(
  ...
  for (uint64_t series_i = 0; series_i < n_series_; ++series_i) {
    for (uint64_t i = 0; i < perms[series_i].size(); ++i) {
  ...
  for (uint64_t series_i = 0; series_i < n_series_; ++series_i) {
    for (uint64_t i = 0; i < perms[series_i].size(); ++i, ++perm_offset) {
