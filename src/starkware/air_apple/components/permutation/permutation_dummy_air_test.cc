...
class PermutationDummyAirTest : public ::testing::Test {
 public:
  ...
  const uint64_t n_interaction_elements;
...
TEST_F(PermutationDummyAirTest, NegativeTest) {
  ...
  const uint64_t bad_col_idx = prng.UniformInt<uint64_t>(0, merged_trace->Width() - 1);
  const uint64_t bad_row_idx = prng.UniformInt<uint64_t>(0, trace_length - 1);
...
TEST_F(PermutationDummyAirTest, IncompatibleInteractionElemsTest) {
  ...
  interaction_elms_vec->Set(
      prng.UniformInt<uint64_t>(0, interaction_elms_vec->Size() - 1),
