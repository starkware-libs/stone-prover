...
std::vector<std::pair<uint64_t, const FieldElementT*>> AddressBasedSort(
  ...
  for (uint64_t i = 0; i < address.size(); ++i) {
...
void ValidateConsecutiveCells(
    ...
    const std::pair<uint64_t, FieldElementT*>& second_cell, const uint64_t index) {
  bool same_address =
...
MemoryComponentProverContext<FieldElementT>::WriteTrace(
  ...
  // Fill trace.
  for (uint64_t i = 0; i < sorted_address_value.size(); ++i) {
...
void MemoryComponentProverContext1<FieldElementT>::WriteTrace(
  ...
  for (uint64_t i = 0; i < address_.size(); ++i) {
