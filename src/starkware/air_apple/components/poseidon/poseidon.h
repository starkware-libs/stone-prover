...
class PoseidonComponent {
  ...
  */
  PoseidonComponent(
      ...
      uint64_t rounds_partial, gsl::span<const uint64_t> r_p_partition,
      ...
      : m_(m),
        ...
        ark_(ark) {
    ASSERT_RELEASE(
        std::accumulate(r_p_partition.begin(), r_p_partition.end(), uint64_t(0)) ==
 ...
 private:
  static const std::vector<uint64_t> InitRPCapacities(gsl::span<const uint64_t> r_p_partition) {
    std::vector<uint64_t> capacities;
    capacities.reserve(r_p_partition.size());
    for (uint64_t size : r_p_partition) {
  ...
  const std::vector<uint64_t> r_p_partition_;
