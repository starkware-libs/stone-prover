...
std::vector<FieldElementT> PoseidonComponent<FieldElementT>::WriteTrace(
  ...
  for (uint64_t part = 0; part < r_p_partition_.size(); ++part) {
    uint64_t first_round = part == 0 ? 0 : m_;
    for (uint64_t round = first_round; round < r_p_partition_[part]; ++round, ++round_key_idx) {
      ...
      if (part < r_p_partition_.size() - 1 && round >= r_p_partition_[part] - m_) {
        const uint64_t index = round - r_p_partition_[part] + m_;
