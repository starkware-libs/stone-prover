...
class ParseToDilutedComponent {
  ...
  */
  ParseToDilutedComponent(
      const std::string& name, const TraceGenerationContext& ctx, gsl::span<const uint64_t> state_rep,
      gsl::span<const uint64_t> dimension_sizes, gsl::span<const uint64_t> dimension_capacities,
      uint64_t n_repetitions, uint64_t diluted_spacing, uint64_t n_instances)
      : state_rep_(state_rep.begin(), state_rep.end()),
        ...
        n_total_bits_(std::accumulate(state_rep.begin(), state_rep.end(), uint64_t(0))),
    ...
    ASSERT_RELEASE(
        n_total_bits_ ==
            std::accumulate(
                dimension_sizes.begin(), dimension_sizes.end(), uint64_t(1), std::multiplies<>()),
  ...
  */
  uint64_t RowIndex(uint64_t m) const;
 ...
 private:
  static std::vector<uint64_t> InitExtendedDimensionSizes(gsl::span<const uint64_t> dimension_sizes) {
    std::vector<uint64_t> extended_dimension_sizes(
  ...
  */
  const std::vector<uint64_t> state_rep_;
  ...
  */
  const std::vector<uint64_t> extended_dimension_sizes_;
  const std::vector<uint64_t> dimension_capacities_;

  /*
    The total period of the parsed output.
  */
  const uint64_t total_period_;

  /*
    The total amount of bits in the input format.
  */
  const uint64_t n_total_bits_;
  ...
  */
  const uint64_t extended_dimensions_total_size_;

  /*
    The number of repetitions.
  */
  const uint64_t n_repetitions_;

  /*
    The space between representation bits.
  */
  const uint64_t diluted_spacing_;
