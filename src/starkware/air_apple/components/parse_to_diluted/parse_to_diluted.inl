...
uint64_t ParseToDilutedComponent<FieldElementT>::RowIndex(uint64_t m) const {
  uint64_t row = 0;
  for (uint64_t i = 0; i < extended_dimension_sizes_.size(); ++i) {
...
void ParseToDilutedComponent<FieldElementT>::WriteTrace(
  ...
  for (uint64_t rep = 0; rep < n_repetitions_; ++rep) {
    for (uint64_t instance = 0; instance < n_instances_; ++instance) {
      for (uint64_t index = n_total_bits_; index < extended_dimensions_total_size_; ++index) {
      ...
      uint64_t bit_index = n_total_bits_ - 1;
      for (int64_t i = n_words_ - 1; i >= 0; --i) {
        ...
        for (int64_t j = state_rep_[i] - 1; j >= 0; --j, --bit_index) {
          ...
          const uint64_t row_bit_index = RowIndex(bit_index);
          if (instance > 0) {
            value +=
                spacing_shift *
