...
void KeccakComponent<FieldElementT>::AppendBytesToKeccakIO(
  ...
  for (uint64_t word = 0; word < kStateSizeInWords; ++word) {
    std::copy(
...
std::vector<FieldElementT> KeccakComponent<FieldElementT>::WriteTrace(
  ...
  for (uint64_t invocation = 0; invocation < n_invocations_; ++invocation) {
  ...
  for (uint64_t round = 0; round < 24; ++round) {
    ...
    for (uint64_t i = 0; i < 5; ++i) {
      for (uint64_t j = 0; j < 5; ++j) {
        const uint64_t pi_i = (3 * i + 2 * j) % 5;
        const uint64_t pi_j = i;
        const uint64_t n = (round / 8) % theta_aux_columns_[pi_i][pi_j].size();
        const uint64_t adjusted_round = round - 8 * n;
        for (uint64_t k = 0; k < 64; ++k) {
          const uint64_t value = state.at(i)[j][k] + parities.at((j + 4) % 5)[k] +
                                 parities.at((j + 1) % 5)[(k + 63) % 64];
          const uint64_t rho_k = (k + kOffsets.at(i)[j]) % 64;
