...
class BitwiseBuiltinProverContext {
 public:
  ...
  BitwiseBuiltinProverContext(
      ...
      uint64_t n_instances, uint64_t diluted_spacing, uint64_t diluted_n_bits, uint64_t total_n_bits,
  ...
  */
  static std::vector<uint64_t> GeneratePartition(
  ...
  */
  std::map<uint64_t, TableCheckCellView<FieldElementT>> GenerateTrimUnpacking(
      const std::string& name, const TraceGenerationContext& ctx,
  ...
  const uint64_t diluted_n_bits_;
  const uint64_t total_n_bits_;
  ...
  */
  const std::vector<uint64_t> partition_;

  /*
    Maps a deviating element of partition_ to the diluted cell that trims it.
  */
  const std::map<uint64_t, TableCheckCellView<FieldElementT>> diluted_cells_trim_unpacking_;
};
