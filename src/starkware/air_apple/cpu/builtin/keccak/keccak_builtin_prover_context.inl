...
void KeccakBuiltinProverContext<FieldElementT>::WriteTrace(
  ...
  for (uint64_t i = 0; i < n_component_instances_; ++i) {
    constexpr uint64_t kPaddingSize =
    ...
    std::vector<std::byte> current_witness(
        ...
        std::byte(0));
    uint64_t pos = 0;
    for (uint64_t idx_in_batch = 0; idx_in_batch < n_invocations_; ++idx_in_batch) {
      uint64_t invocation = i * n_invocations_ + idx_in_batch;
    ...
    const auto io = keccak_component_.WriteTrace(
        ...
        i, trace);
    constexpr uint64_t kInputOutputLength = 2 * KeccakComponent<FieldElementT>::kStateSizeInWords;
    ASSERT_RELEASE(io.size() == n_invocations_ * kInputOutputLength, "");
    for (uint64_t idx_in_batch = 0; idx_in_batch < n_invocations_; ++idx_in_batch) {
      uint64_t invocation = i * n_invocations_ + idx_in_batch;
      for (uint64_t index = 0; index < kInputOutputLength; ++index) {
