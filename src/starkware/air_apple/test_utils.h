...
class DummyAir : public Air {
 public:
  ...
  std::unique_ptr<CompositionPolynomial> CreateCompositionPolynomial(
    ...
    for (uint64_t i = 0; i < periodic_columns.size(); ++i) {
...
Trace DrawRandomTrace(uint64_t width, uint64_t height, const Field& field, Prng* prng);
...
void TestAirConstraint(
    const Air& air, const Field& field, uint64_t trace_length, uint64_t constraint_idx,
