...
TEST(AirTestUtils, ComputeCompositionDegree) {
  ...
  const uint64_t trace_length = Pow2(5);
...
TEST(AirTestUtils, DrawRandomTrace) {
  // Initialize test parameters.
  Prng prng;
  const uint64_t width = prng.UniformInt(0, 5);
  const uint64_t height = prng.UniformInt(0, 5);
...
TEST(AirTestUtils, DomainPredicateToList) {
  const auto predicate = [](const uint64_t i) { return (i % 5 == 0) || (i % 3 == 0); };

  EXPECT_EQ(
      DomainPredicateToList(predicate, 20), std::vector<uint64_t>({0, 3, 5, 6, 9, 10, 12, 15, 18}));
}
