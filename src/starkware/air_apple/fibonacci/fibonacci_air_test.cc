...
TEST(FibonacciAirTest, CompositionEndToEnd) {
  using AirT = FibonacciAir<FieldElementT>;

  Prng prng;
  const uint64_t trace_length = 512;
  const uint64_t fibonacci_claim_index = 500;
  const FieldElementT private_input(FieldElementT::RandomElement(&prng));
  const FieldElementT claimed_fib(FibonacciAir<FieldElementT>::PublicInputFromPrivateInput(
      private_input, fibonacci_claim_index));
  const AirT air(trace_length, fibonacci_claim_index, claimed_fib);
  ...
  // Construct trace.
  const Trace trace = AirT::GetTrace(private_input, trace_length, fibonacci_claim_index);

  // Verify composition degree.
  EXPECT_EQ(
      ComputeCompositionDegree(air, trace, random_coefficients),
      air.GetCompositionPolynomialDegreeBound() - 2);
