...
int64_t ComputeCompositionDegree(
  ...
  const uint64_t coset_size = trace.GetColumn(0).Size();
  const uint64_t evaluation_domain_size =
      Pow2(Log2Ceil(air.GetCompositionPolynomialDegreeBound() * num_of_cosets));
  const uint64_t n_cosets = SafeDiv(evaluation_domain_size, coset_size);
  ...
  for (uint64_t i = 0; i < trace.Width(); ++i) {
...
Trace DrawRandomTrace(const uint64_t width, const uint64_t height, const Field& field, Prng* prng) {
...
bool TestOneConstraint(const Air& air, const Trace& trace, uint64_t constraint_id, Prng* prng) {
  ASSERT_RELEASE(
...
*/
bool TestConstraintRange(const Air& air, const Trace& trace, uint64_t start, uint64_t end, Prng* prng) {
  ASSERT_RELEASE(
      trace.Width() > 0 && trace.GetColumn(0).Size() > 0, "Trace is expected to be not empty.");
  ...
  FieldElementVector coefficients =
      FieldElementVector::Make(air.NumRandomCoefficients(), field.Zero());
  for (uint64_t constraint_id = start; constraint_id < end; ++constraint_id) {
    coefficients.Set(constraint_id, field.RandomElement(prng));
  }
  const uint64_t degree = ComputeCompositionDegree(air, trace, coefficients);
  if (degree == uint64_t(-1)) {
...
std::set<uint64_t> GetFailingConstraints(const Air& air, const Trace& trace, Prng* prng) {
  const uint64_t n_constraints = air.GetNumConstraints();
...
std::set<uint64_t> GetFailingConstraintsBinarySearch(const Air& air, const Trace& trace, Prng* prng) {
  const uint64_t n_constraints = air.GetNumConstraints();
...
void GetTraceRow(const Trace& trace, const uint64_t row_idx, const FieldElementSpan& dst) {
  ASSERT_RELEASE(dst.Size() == trace.Width(), "Span size must be equal to trace width.");
  for (uint64_t column = 0; column < trace.Width(); column++) {
    dst.Set(column, trace.GetColumn(column)[row_idx]);
  }
}

void SetTraceRow(Trace* trace, const uint64_t row_idx, const ConstFieldElementSpan& src) {
...
void ApplyManipulation(
    Trace* trace, const uint64_t row_idx,
  ...
  const uint64_t trace_length = trace->GetColumn(0).Size();
...
void TestAirConstraint(
    const Air& air, const Field& field, const uint64_t trace_length, const uint64_t constraint_idx,
    const std::vector<uint64_t>& domain_indices,
  ...
  for (const uint64_t i : domain_indices) {
    // Manipulate lines.
  ...
  const uint64_t bad_row_idx = domain_indices[prng->UniformInt(uint64_t(0), (unsigned long long)domain_indices.size() - 1)];
...
std::vector<uint64_t> DomainPredicateToList(
    const std::function<bool(uint64_t)>& predicate, uint64_t n_elements) {
