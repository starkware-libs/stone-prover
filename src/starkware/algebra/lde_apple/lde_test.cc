...
class LdeTest : public ::testing::Test {
 public:
  ...
  BasesT GetBases(uint64_t log_n, const FieldElementT& offset) { return BasesT(log_n, offset); }
};
...
template <typename FieldElementT>
void IdentityTest(uint64_t log_domain_size) {
  const Field field = Field::Create<FieldElementT>();
  Prng prng;
...
void EvalAtPointTest(const uint64_t log_domain_size) {
  const Field field = Field::Create<FieldElementT>();
  Prng prng;
