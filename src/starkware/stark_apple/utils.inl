...
TableProverFactory GetTableProverFactory(
    ProverChannel* channel, uint64_t field_element_size_in_bytes, uint64_t n_tasks_per_segment,
    uint64_t n_out_of_memory_merkle_layers, uint64_t n_verifier_friendly_commitment_layers,
  ...
  return [channel, field_element_size_in_bytes, n_tasks_per_segment, n_out_of_memory_merkle_layers,
          n_verifier_friendly_commitment_layers, commitment_hashes](
             uint64_t n_segments, uint64_t n_rows_per_segment,
             uint64_t n_columns) -> std::unique_ptr<TableProver> {
    ...
    if (n_rows_per_segment >= n_tasks_per_segment &&
      ...
      uint64_t log_n_tasks_per_segment = SafeLog2(n_tasks_per_segment);
