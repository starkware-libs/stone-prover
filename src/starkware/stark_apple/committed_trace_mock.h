...
class CommittedTraceProverMock : public CommittedTraceProverBase {
 public:
  MOCK_CONST_METHOD0(NumColumns, uint64_t());
  ...
  */
  ...
  MOCK_CONST_METHOD1(
      DecommitQueries, void(gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>>));
  MOCK_CONST_METHOD3(
...
class CommittedTraceVerifierMock : public CommittedTraceVerifierBase {
 public:
  ...
  MOCK_CONST_METHOD1(
      VerifyDecommitment,
      FieldElementVector(gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>>));
};
