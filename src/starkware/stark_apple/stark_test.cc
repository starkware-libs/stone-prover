...
std::unique_ptr<TableProver> MakeTableProver(
    uint64_t n_segments, uint64_t n_rows_per_segment, uint64_t n_columns, ProverChannel* channel,
    uint64_t n_tasks_per_segment, uint64_t n_layer, uint64_t n_verifier_friendly_commitment_layers) {
...
std::vector<uint64_t> DrawFriStepsList(const uint64_t log_degree_bound, Prng* prng) {
  if (prng == nullptr) {
    return {3, 3, 1, 1};
  }
  std::vector<uint64_t> res;
  for (uint64_t curr_sum = 0; curr_sum < log_degree_bound;) {
    res.push_back(prng->UniformInt<uint64_t>(1, log_degree_bound - curr_sum));
    curr_sum += res.back();
  }
  ASSERT_RELEASE(
...
StarkParameters GenerateParameters(
    std::unique_ptr<Air> air, Prng* prng, uint64_t proof_of_work_bits = 15,
  ...
  const auto log_n_cosets =
      (prng == nullptr ? 4 : prng->UniformInt<uint64_t>(Log2Ceil(degree_bound), 6));
  const uint64_t log_coset_size = Log2Ceil(trace_length);
  const uint64_t n_cosets = Pow2(log_n_cosets);
  const uint64_t log_evaluation_domain_size = log_coset_size + log_n_cosets;

  // Fri steps - hard-coded pattern for this tests suit.
  const uint64_t log_degree_bound = log_coset_size;
  const std::vector<uint64_t> fri_step_list = DrawFriStepsList(log_degree_bound, prng);
 ...
 public:
  explicit StarkTest(bool use_random_values = true)
    ...
    table_prover_factory = [this](
                               uint64_t n_segments, uint64_t n_rows_per_segment, uint64_t n_columns) {
      const uint64_t n_verifier_friendly_commitment_layers = 0;
...
TYPED_TEST(StarkTestConstSeed, Regression_Statistical) {
  ...
  const uint64_t hash_id = typeid(TypeParam).hash_code();
  const uint64_t blake_id = typeid(Blake2s256).hash_code();
  const uint64_t keccak_id = typeid(Keccak256).hash_code();
  ...
  const std::map<uint64_t, uint64_t> expected_field_element_count = {
...
TYPED_TEST(FibonacciStarkTest, StarkWithFriProverBadTrace) {
  ...
  auto bad_column = this->prng.template UniformInt<uint64_t>(0, 1);
  auto bad_index = this->prng.template UniformInt<uint64_t>(0, this->fibonacci_claim_index - 1);
...
TYPED_TEST(FibonacciStarkTest, StarkWithFriProverWrongNumberOfLayers) {
  this->GetStarkParams().fri_params->fri_step_list = std::vector<uint64_t>(5, 1);
...
TYPED_TEST(FibonacciStarkTest, ChangeProofContent) {
  ...
  modified_proof[this->prng.template UniformInt<uint64_t>(0, modified_proof.size() - 1)] ^=
      std::byte(1);
