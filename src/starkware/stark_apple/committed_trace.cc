...
std::unique_ptr<CachedLdeManager> CreateLdeManager(
  ...
  uint64_t log_cosets = SafeLog2(evaluation_domain.NumCosets());
...
CommittedTraceProver::CommittedTraceProver(
    ...
    MaybeOwnedPtr<const ListOfCosets> evaluation_domain, uint64_t n_columns,
...
void CommittedTraceProver::DecommitQueries(
    gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>> queries) const {
  const Field field = evaluation_domain_->GetField();
...
void CommittedTraceProver::EvalMaskAtPoint(
  ...
  std::map<uint64_t, std::vector<std::pair<int64_t, uint64_t>>> columns;
  for (uint64_t mask_index = 0; mask_index < mask.size(); ++mask_index) {
  ...
  for (const auto& [column_index, offsets] : columns) {
    ...
    for (uint64_t i = 0; i < offsets.size(); ++i) {
      const uint64_t mask_index = offsets[i].second;
