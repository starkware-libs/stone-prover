...
TEST(OutOfDomainSampling, ProveOods) {
  ...
  const uint64_t trace_length = 16;
  const uint64_t n_cosets = 4;
  const uint64_t mask_size = 7;
  const uint64_t n_columns = 11;
  const uint64_t n_breaks = 2;
...
void FibonacciOodsEndToEnd(bool use_extension_field, const FieldElementT& secret, Prng* prng) {
  const uint64_t trace_length = 16;
  const uint64_t fibonacci_claim_index = 12;
  const uint64_t n_cosets = 8;
  ...
  // Commit on trace.
  const uint64_t n_columns = fibonacci_air.NumColumns();
  const uint64_t n_layers = 0;
  ...
  const uint64_t num_random_coefficients_required = fibonacci_air.NumRandomCoefficients();
  ...
  // Break polynomial.
  const uint64_t n_breaks = oracle_prover.ConstraintsDegreeBound();
