...
void TestEndToEnd(
  ...
  const uint64_t n_columns = 7;
  const uint64_t mask_size = 12;
  const uint64_t n_out_of_memory_merkle_layers = 0;
  ...
  const uint64_t n_queries = 12;
  ASSERT_RELEASE(n_queries >= 2, "Not enough queries for test");

  std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> queries;
  ...
  queries.push_back(queries[prng.UniformInt<uint64_t>(0, n_queries - 2)]);
  ...
  if (verify_decommit_in_base_field) {
    ...
    for (uint64_t query_index = 0; query_index < queries.size(); ++query_index) {
