...
class CommittedTraceProverBase {
  ...
  */
  virtual void DecommitQueries(
      gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>> queries) const = 0;
...
class CommittedTraceProver : public CommittedTraceProverBase {
 public:
  ...
  void DecommitQueries(
      gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>> queries) const override;
