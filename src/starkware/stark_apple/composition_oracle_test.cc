...
class CompositionOracleProverTester {
 public:
  CompositionOracleProverTester(
      uint64_t trace_length, uint64_t n_cosets, uint64_t n_columns, uint64_t n_traces,
    ...
    // Create traces mocks.
    for (uint64_t trace_i = 0; trace_i < n_traces; ++trace_i) {
  ...
  void TestEvalComposition(uint64_t degree_bound);
...
*/
void CompositionOracleProverTester::TestEvalComposition(uint64_t degree_bound) {
  const uint64_t task_size = 32;
  ...
  const uint64_t log_cosets = SafeLog2(evaluation_domain.NumCosets());
  ...
  for (uint64_t trace_i = 0; trace_i < n_traces; ++trace_i) {
    ...
    for (uint64_t column_i = 0; column_i < n_columns; ++column_i) {
...
void CompositionOracleProverTester::TestDecommitQueries() {
  ...
  std::vector<std::pair<int64_t, uint64_t>> mask;
  for (uint64_t trace_i = 0; trace_i < n_traces; ++trace_i) {
    const uint64_t masks_items_in_trace = prng.UniformInt<uint64_t>(1, 5);
    const uint64_t column_offset = n_columns * trace_i;
    for (uint64_t i = 0; i < masks_items_in_trace; ++i) {
      mask.emplace_back(i, prng.UniformInt<uint64_t>(0, n_columns - 1) + column_offset);
    }

    EXPECT_CALL(
        *traces[trace_i],
        DecommitQueries(Property(
            &gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>>::size, masks_items_in_trace)));
  }
...
void CompositionOracleProverTester::TestInvalidMask() {
  ...
  for (uint64_t i = 0; i <= n_traces * n_columns; ++i) {
...
void VerifierTestVerifyDecommitment(
    uint64_t trace_length, uint64_t n_cosets, uint64_t n_columns, uint64_t n_traces,
    MultiplicativeGroupOrdering order, uint64_t n_queries) {
  ...
  for (uint64_t trace_i = 0; trace_i < n_traces; ++trace_i) {
    ...
    EXPECT_CALL(
        *traces[trace_i], VerifyDecommitment(Property(
                              &gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>>::size,
                              n_queries * masks_items_in_trace)))
        .WillOnce(Invoke([](gsl::span<const std::tuple<uint64_t, uint64_t, uint64_t>> queries) {
  ...
  for (uint64_t i = 0; i < n_queries - 1; ++i) {
