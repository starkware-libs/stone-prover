...
*/
std::pair<uint64_t, uint64_t> ColumnToTraceColumn(uint64_t column, const std::vector<uint64_t>& widths) {
  for (uint64_t trace_i = 0; trace_i < widths.size(); ++trace_i) {
...
*/
std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> QueriesToTraceQueries(
    gsl::span<const std::pair<uint64_t, uint64_t>> queries,
    gsl::span<const std::pair<int64_t, uint64_t>> trace_mask, const uint64_t trace_length) {
  const uint64_t n_trace_bits = SafeLog2(trace_length);

  std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> trace_queries;
...
FieldElementVector CompositionOracleProver::EvalComposition(uint64_t task_size) const {
  ...
  const uint64_t n_segments = ConstraintsDegreeBound();
  ASSERT_RELEASE(
  ...
  for (uint64_t coset_index = 0; coset_index < n_segments; coset_index++) {
    ...
    for (uint64_t trace_idx = 0; trace_idx < traces_.size(); ++trace_idx) {
      ...
      for (uint64_t col_idx = 0; col_idx < coset_columns_eval->size(); ++col_idx) {
    ...
    const uint64_t coset_natural_index = BitReverse(coset_index, log_n_cosets);
...
void CompositionOracleProver::DecommitQueries(
  ...
  for (uint64_t trace_i = 0; trace_i < traces_.size(); ++trace_i) {
...
void CompositionOracleProver::EvalMaskAtPoint(
  ...
  std::vector<uint64_t> mask_offset_in_trace(traces_.size());
  for (uint64_t mask_i = 0; mask_i < mask_.size(); ++mask_i) {
...
uint64_t CompositionOracleProver::Width() const { return Sum(GetWidths(traces_)); }
...
uint64_t CompositionOracleVerifier::Width() const { return Sum(GetWidths(traces_)); }
