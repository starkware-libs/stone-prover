...
struct StarkParameters {
  StarkParameters(
      const Field& field, bool use_extension_field, uint64_t n_evaluation_domain_cosets,
      uint64_t trace_length, MaybeOwnedPtr<const Air> air, MaybeOwnedPtr<FriParameters> fri_params);

  uint64_t TraceLength() const { return Pow2(evaluation_domain.Bases().NumLayers()); }
  FieldElement TraceCosetOffset() const { return field.One(); }
  uint64_t NumCosets() const { return evaluation_domain.NumCosets(); }
  uint64_t NumColumns() const { return (air->NumColumns()); }
...
struct StarkProverConfig {
  ...
  */
  uint64_t table_prover_n_tasks_per_segment;
  ...
  */
  uint64_t n_out_of_memory_merkle_layers;
...
class StarkProver {
  ...
  */
  void ValidateFirstTraceSize(uint64_t n_rows, uint64_t n_columns);
...
class StarkVerifier {
  ...
  */
  CommittedTraceVerifier ReadTraceCommitment(
      uint64_t n_columns, bool should_verify_base_field = false);
