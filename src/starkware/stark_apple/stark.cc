...
std::unique_ptr<CompositionPolynomial> CreateCompositionPolynomial(
  ...
  uint64_t num_random_coefficients_required = air.NumRandomCoefficients();
  ...
  for (uint64_t i = 0; i < num_random_coefficients_required; ++i) {
...
uint64_t GetFriExpectedDegreeBound(const FriParameters& fri_params) {
  ...
  for (const uint64_t fri_step : fri_params.fri_step_list) {
...
FieldElementVector GetInteractionElements(
    const uint64_t n_interaction_elements, const Field& field, Channel* channel) {
  auto interaction_elms_vec = FieldElementVector::Make(field);
  interaction_elms_vec.Reserve(n_interaction_elements);
  for (uint64_t i = 0; i < n_interaction_elements; ++i) {
...
static std::unique_ptr<FftBases> GenerateCompositionBases(const Field& field, const Air& air) {
  const uint64_t log_size = SafeLog2(air.GetCompositionPolynomialDegreeBound());
...
void StarkParameters::VerifyCompatibleDomains() {
  ...
  for (uint64_t i = 0; i < n_relevant_cosets; i++) {
    ASSERT_RELEASE(
...
StarkParameters::StarkParameters(
    const Field& field, const bool use_extension_field, uint64_t n_evaluation_domain_cosets,
    uint64_t trace_length, MaybeOwnedPtr<const Air> air, MaybeOwnedPtr<FriParameters> fri_params)
    : field(field),
...
StarkParameters StarkParameters::FromJson(
  ...
  const uint64_t log_trace_length = SafeLog2(trace_length);
  const uint64_t log_n_cosets = json["log_n_cosets"].AsSizeT();
...
StarkProverConfig StarkProverConfig::FromJson(const JsonValue& json) {
  ...
  const uint64_t table_prover_n_tasks_per_segment =
      json["table_prover_n_tasks_per_segment"].AsSizeT();
  const uint64_t n_out_of_memory_merkle_layers = json["n_out_of_memory_merkle_layers"].AsSizeT();

  const JsonValue fri_prover_config = json["fri_prover"];
  uint64_t max_non_chunked_layer_size = FriProverConfig::kDefaultMaxNonChunkedLayerSize;
  uint64_t n_chunks_between_layers = FriProverConfig::kDefaultNumberOfChunksBetweenLayers;
  uint64_t log_n_max_in_memory_fri_layer_elements = FriProverConfig::kAllInMemoryLayers;
...
void StarkProver::ValidateFirstTraceSize(const uint64_t n_rows, const uint64_t n_columns) {
  ASSERT_RELEASE(
...
void StarkProver::ProveStark(std::unique_ptr<TraceContext> trace_context) {
  ...
  const uint64_t trace_length = trace.Length();
  const uint64_t first_trace_width = trace.Width();
  ...
  if (interaction_params.has_value()) {
    ...
    const uint64_t trace_width = first_trace_width + interaction_trace.Width();
...
CommittedTraceVerifier StarkVerifier::ReadTraceCommitment(
    const uint64_t n_columns, bool should_verify_base_field) {
  ASSERT_RELEASE(
